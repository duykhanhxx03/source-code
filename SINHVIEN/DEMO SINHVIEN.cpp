...
{
	filein >> date.ngay;
	filein.seekg(1, ios::cur); // dịch sang phải 1 byte để bỏ qua kí tự '/'
	filein >> date.thang;
	filein.seekg(1, ios::cur); // dịch sang phải 1 byte để bỏ qua kí tự '/'
	filein >> date.nam;
}
...
void Doc_Thong_Tin_1_Sinh_Vien(ifstream &filein, SINHVIEN &sv)
{
	...
	filein.seekg(1, ios::cur); // tại vị trí hiện tại dịch sang phải 1 byte để bỏ khoảng trắng ' '
	// đọc mã sinh viên
	getline(filein, sv.masv, ','); // đọc đến dấu ',' thì dừng
	filein.seekg(1, ios::cur); // tại vị trí hiện tại dịch sang phải 1 byte để bỏ khoảng trắng ' '
	// đọc thông tin ngày tháng năm sinh
	Doc_Ngay_Thang_Nam(filein, sv.ngaysinh);
	filein.seekg(1, ios::cur); // tại vị trí hiện tại dịch sang phải 2 byte để bỏ dấu phẩy và khoảng trắng ' '
	// đọc điểm trung bình
	filein >> sv.dtb;
